...
class ProductBenchmark(Base):
    """
    Mirrors CompetitorBenchmark's structure and scoring discipline
    (BenchmarkEngine.calculate_product_benchmarks reuses the same
    per-entity _components_for/_score_from_components logic), keyed on
    (client_id, product_entity_id) instead of (client_id,
    ...
    product_entity_id = Column(UUID(as_uuid=True), ForeignKey("entities.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # Accuracy & Explainability columns
