...
class BenchmarkEngine:
    ...
    def _preload_score_inputs(self, db: Session, client_id: str, entity_ids: List[Any], lookback_date, log):
        ...
        single-purpose calculate_competitor_benchmarks so it can be reused by
        calculate_product_benchmarks without a second scoring engine (Product
        Compare task, Part 1.2) — parameterized by an arbitrary `entity_ids`
        list instead of being hardcoded to "client brand + its competitors".
        ...
        exec_reps = db.query(ExecutiveReputationScore).filter(
            ...
            ExecutiveReputationScore.entity_id.in_(entity_ids),
    ...
    def calculate_product_benchmarks(
        ...
        """
        Product Compare (Part 1.2): scores every tracked product entity for
        this client -- both the client's own products (parent_entity_id ->
        a 'brand' entity) and a competitor's products (parent_entity_id ->
        a 'competitor' entity) -- via the identical
        _preload_score_inputs/_score_from_components pipeline
        calculate_competitor_benchmarks uses above, just fed the client's
        product entity ids instead of [brand] + competitors, and writing to
        ProductBenchmark instead of CompetitorBenchmark. No second scoring
        engine.

        Deliberately does not touch BenchmarkStateMachine: that FSM tracks
        company-level benchmark processing on the Client row
        (benchmark_processing_status etc.) for calculate_competitor_benchmarks'
        own pipeline stage. A client with zero tracked products yet is not a
        company-level benchmark failure and must not flip that state.

        executive_reputation_score is always MISSING_EXECUTIVE_SCORE here --
        not fabricated, a product entity has no executive to score -- kept
        as a real column (not dropped) only because the task asks this table
        to mirror CompetitorBenchmark's shape column-for-column.
        """
        t0 = time.perf_counter()
        rid = run_id or uuid.uuid4().hex
        bid = batch_id or uuid.uuid4().hex[:12]
        ...
        log = logger.bind(
            ...
            client_id=client_id,
            attempt=attempt,
            processing_stage="PRODUCT_BENCHMARK_CALCULATION"
        )
        log.info("product_benchmark_calculation_started")

        client = db.query(Client).filter(Client.id == client_id).first()
        if not client:
            log.error("product_benchmark_client_not_found")
            raise ValueError(f"Client {client_id} not found")

        now_utc = datetime.datetime.now(datetime.timezone.utc)
        lookback_date = now_utc - datetime.timedelta(days=30)

        # Tracked product entities for this client, each required to point at
        # a parent brand/competitor entity that also belongs to this client
        # (tenant isolation: a product cannot be scored under a parent it
        # doesn't actually belong to).
        ParentEntity = aliased(Entity)
        products = db.query(Entity).join(
            ParentEntity, Entity.parent_entity_id == ParentEntity.id
        ).filter(
            Entity.client_id == client_id,
            Entity.entity_type == "product",
            ParentEntity.client_id == client_id,
            ParentEntity.entity_type.in_(("brand", "competitor")),
        ).all()

        if not products:
            log.info("product_benchmark_no_products_skipped")
            return

        all_entity_ids = [p.id for p in products]
        inputs = self._preload_score_inputs(db, client_id, all_entity_ids, lookback_date, log)
        mentions_map = inputs["mentions_map"]
        sentiment_map = inputs["sentiment_map"]
        risk_map = inputs["risk_map"]
        ...
        for p in products:
            try:
                mentions = mentions_map.get(p.id, 0) or 0
                avg_sentiment = sentiment_map.get(p.id, 0.0) or 0.0
                avg_risk = risk_map.get(p.id, 0.0) or 0.0

                sov = (mentions / total_mentions * 100.0) if total_mentions > 0 else 0.0

                components = components_for(p.id)
                rep_score, active_weight = self._score_from_components(components, weights)

                # Same honest-evidence discipline as calculate_competitor_benchmarks:
                # no evidence must not yield a plausible-looking number.
                has_evidence = rep_score is not None
                ...
                log.error("product_benchmark_score_failed", product_id=str(p.id), error=str(exc))

        # Rank across every scored product entity (own + competitors'), same
        # "no evidence -> unranked" rule as calculate_competitor_benchmarks.
        scored_entities = [
            (res["product_entity_id"], res["reputation_score"])
            for res in benchmark_results if res["has_evidence"]
        ]
        scored_entities.sort(key=lambda x: (-x[1], str(x[0])))
        ranks = {ent_id: i + 1 for i, (ent_id, _) in enumerate(scored_entities)}

        log.info(
            "product_benchmark_scores_computed",
            products_scored=len(scored_entities),
            products_unranked=sum(1 for r in benchmark_results if not r["has_evidence"]),
        )

        for res in benchmark_results:
            eid = res["product_entity_id"]
            rep_score = res["reputation_score"]
            entity_rank = ranks.get(eid, 0)
            latency_ms = (time.perf_counter() - t0) * 1000
            ...
            stmt = insert(ProductBenchmark).values(
                id=uuid.uuid4(),
                client_id=client_id,
                product_entity_id=eid,
                reputation_score=rep_score,
                executive_reputation_score=self.MISSING_EXECUTIVE_SCORE,
                sentiment_score=res["sentiment_score"],
                risk_score=res["risk_score"],
                visibility_score=res["visibility_score"],
                share_of_voice=res["share_of_voice"],
                ...
                constraint="uq_product_benchmark_run",
                set_={
                    "reputation_score": rep_score,
                    "executive_reputation_score": self.MISSING_EXECUTIVE_SCORE,
                    "sentiment_score": res["sentiment_score"],
